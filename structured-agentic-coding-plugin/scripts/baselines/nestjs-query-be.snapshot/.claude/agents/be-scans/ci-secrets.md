...
**What to check:** Deploy workflow runs `bun run migration:run` before the new Cloud Run revision receives traffic.

**Scan:**
```
Grep pattern: "migration:run|bun run migration:run"
     path:    ./.github/workflows
     output_mode: content
     -B:      5
     -A:      10
