...
# Test NestJS Query BE Masterplan Architect
...
- Include build commands: `bun run build` for BE, `__FE_BUILD__` for FE
- Include format commands: `bun run format` for BE, `__FE_FORMAT__` for FE
- Include test commands: `bun run test` for BE, `__FE_TEST__` for FE
