...
# Test Angular Dotnet Masterplan Reviewer
...
1. Determine which scan playbooks apply based on file paths:
   - Backend files -> read relevant backend scan playbooks from `backend/.claude/agents/` (if they exist)
   - Frontend files -> read relevant frontend scan playbooks from `frontend/.claude/agents/` (if they exist)
