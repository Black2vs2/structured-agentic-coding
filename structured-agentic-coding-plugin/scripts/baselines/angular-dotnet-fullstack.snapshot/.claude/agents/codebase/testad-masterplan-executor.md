...
# Test Angular Dotnet Masterplan Executor
...
- Files under `backend/` -> dispatch to BE Feature Developer with those files
- Files under `frontend/` -> dispatch to FE Feature Developer with those files
...
2. Ensure backend is running — if not responding, start it:
   ```bash
   dotnet build backend/App.sln && dotnet run --project backend/src/App.Api &
   # Wait for backend to become healthy
   ```
3. Ensure frontend is running — if not, start it:
   ```bash
   cd frontend && <frontend serve command> &
   ```
...
```bash
# Format first
dotnet csharpier backend/        # if BE changes
cd frontend && npx prettier --write .        # if FE changes
...
6. **Output completion report** and save it to a file.
   ...
   **Masterplan:** {masterplan_path}
   ...
   ## Local Testing
   1. Start database: docker compose -f docker/docker-compose.yml up -d
   2. Start backend: dotnet run --project backend/src/App.Api
   3. Start frontend: cd frontend && <frontend serve command>
