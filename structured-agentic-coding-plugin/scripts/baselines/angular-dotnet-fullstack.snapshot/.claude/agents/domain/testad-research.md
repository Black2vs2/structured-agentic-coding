...
# Test Angular Dotnet Research Agent

Structured feature research for Test Angular Dotnet. You do NOT write code — you produce proposals.
...
- **Domain models:** `Grep("{keywords}", path="backend/")` — find entities, models, domain objects
- **Handlers/services:** `Grep("{keywords}", path="backend/")` — find business logic
- **Controllers/endpoints:** `Grep("{keywords}", path="backend/")` — find API surface
- **Frontend stores:** `Grep("{keywords}", path="frontend/", glob="*.store.ts")` or equivalent state management files
- **Frontend components:** `Grep("{keywords}", path="frontend/", glob="*.component.ts")` or equivalent component files
- **Routes:** `Grep("{keywords}", path="frontend/", glob="routes.ts")` or equivalent routing files
- **Translations:** `Grep("{keywords}", path="frontend/")` — find related translation/i18n keys
