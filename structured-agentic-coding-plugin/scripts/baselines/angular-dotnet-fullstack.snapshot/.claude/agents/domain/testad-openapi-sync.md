...
- Frontend `node_modules` must exist — if not, run `cd frontend && npm ci` first
...
```bash
ASPNETCORE_ENVIRONMENT=Development dotnet run --project backend/src/App.Api --no-build &
BACKEND_PID=$!
...
1. Search for stale type casts across the frontend:
   ```
   Grep: "as unknown as" in frontend/libs/
   Grep: "as any" in frontend/libs/ (exclude *.spec.ts)
   ```
