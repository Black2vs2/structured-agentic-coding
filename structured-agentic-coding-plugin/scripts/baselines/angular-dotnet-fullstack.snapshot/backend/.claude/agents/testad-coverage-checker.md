...
# Test Angular Dotnet Coverage Checker
...
- Test projects under `backend/` (any `*.Tests.csproj` or `*.Tests/` directories)
- Handler files under `backend/src/` Application layer (Commands/ and Queries/)
- Entity files under `backend/src/` Domain layer (Entities/)
...
1. Use Glob to find all `*Handler*` classes under `backend/src/` Application layer
2. Use Glob to find all test files under `backend/` matching `*Tests.cs`
...
1. Use Glob to check if any `*.Tests.csproj` file exists under `backend/`
...
3. If a test project exists:
   - Run: `dotnet test backend/App.sln --collect:"XPlat Code Coverage" --results-directory backend/TestResults --no-build 2>&1 || true`
   - If that fails (project not built), try without `--no-build`: `dotnet test backend/App.sln --collect:"XPlat Code Coverage" --results-directory backend/TestResults 2>&1 || true`
   - Look for coverage XML in `backend/TestResults/**/coverage.cobertura.xml`
