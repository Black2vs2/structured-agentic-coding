...
# Test Refine NestJS Query FE Documentation Enforcer
...
- Scan playbooks in `__BE_DIR__/.claude/agents/be-scans/` and `./.claude/agents/fe-scans/`
...
- Layer-level: Glob for project directories under `__BE_DIR__/src/*/`
...
- Core libraries: Glob for library directories under `./` (e.g., `./libs/core/*/`, `./src/app/core/*/`)
- Feature modules: Glob for feature directories (e.g., `./libs/features/*/`, `./src/app/features/*/`)
- Page modules: Glob for page directories (e.g., `./libs/pages/*/`, `./src/app/pages/*/`)
...
1. **Discover backend significant directories:**
   - Layer-level: Glob for project directories under `__BE_DIR__/src/*/` (or `__BE_DIR__/*/` depending on project structure)
