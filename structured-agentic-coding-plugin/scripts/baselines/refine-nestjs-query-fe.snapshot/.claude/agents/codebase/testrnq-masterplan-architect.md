...
# Test Refine NestJS Query FE Masterplan Architect
...
1. **Scan existing pages** for UI patterns relevant to the feature:
   - `Glob("./**/pages/**/*.component.ts")` or equivalent — find similar pages
   - `Glob("./**/features/**/*.component.ts")` or equivalent — find shared UI components
...
- Include build commands: `__BE_BUILD__` for BE, `bun run build` for FE
- Include format commands: `__BE_FORMAT__` for BE, `bun run format` for FE
- Include test commands: `__BE_TEST__` for BE, `bun run test` for FE
