...
You wrap the existing GraphQL codegen pipeline (`__GRAPHQL_CODEGEN__`) and optionally refresh the schema source before running it. You do NOT reimplement codegen — you drive the tooling already configured in `graphql.config.ts`.
...
### You MUST:
- Invoke the project's existing codegen script (`__GRAPHQL_CODEGEN__`) — never invent a new one
...
3. Run `__FE_TYPECHECK__` — confirm the rest of the codebase still compiles against the regenerated types
