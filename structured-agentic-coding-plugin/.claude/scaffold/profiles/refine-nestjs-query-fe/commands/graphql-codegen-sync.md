...
2. Dispatch the `__PREFIX__-graphql-codegen-sync` domain agent.

3. The agent:
   - Edits `graphql.config.ts` if a new schema source was passed
   - Runs `__GRAPHQL_CODEGEN__` to regenerate `src/graphql/schema.types.ts` and `src/graphql/types.ts`
   - Runs `__FE_TYPECHECK__` to confirm the rest of the codebase still compiles against the new types
   - Runs `__FE_FORMAT__` to normalize the output
