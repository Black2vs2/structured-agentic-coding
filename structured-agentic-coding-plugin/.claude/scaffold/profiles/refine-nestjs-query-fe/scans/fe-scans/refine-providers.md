...
**Scan:**
```
Glob pattern: __FE_DIR__/src/resources/*/
```
Then read `__FE_DIR__/src/App.tsx` and compare the `resources` array entries (by `name`) against the directory names.
...
Grep pattern: "\\b(fetch|axios)\\s*\\("
     ...
     output_mode: content
```
Also scan `__FE_DIR__/src/hooks` and `__FE_DIR__/src/components`.
