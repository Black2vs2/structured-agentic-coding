...
# __PROJECT_NAME__ Frontend Test Generator
...
- **Bash:** For running tests:
  - `cd __FE_DIR__ && npx vitest run {path-to-spec} --reporter=verbose` (run single spec)
  - `__FE_TEST__` (run all tests)
...
```
Glob("__FE_DIR__/libs/**/*.store.spec.ts")  # for stores
Glob("__FE_DIR__/libs/**/*.service.spec.ts")  # for services
Glob("__FE_DIR__/libs/**/*.component.spec.ts")  # for components
```
...
```bash
cd __FE_DIR__ && npx vitest run {path-to-spec} --reporter=verbose
```
...
- `.spec.ts` files co-located with source files under `__FE_DIR__/libs/` or `__FE_DIR__/src/`
