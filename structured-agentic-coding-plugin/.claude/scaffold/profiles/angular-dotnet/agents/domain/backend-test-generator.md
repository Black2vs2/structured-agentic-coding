...
# __PROJECT_NAME__ Backend Test Generator
...
- **Bash:** For running tests and ensuring database:
  - `__DB_START__` (integration tests need DB)
  - `__BE_TEST__`
  - `dotnet test __BE_DIR__/tests/{ProjectName}/{ProjectName}.csproj --filter "FullyQualifiedName~{TestClass}"`
...
```
Glob("__BE_DIR__/tests/{TestProject}/**/*Tests.cs")
```
...
```bash
dotnet test __BE_DIR__/tests/{TestProject}/{TestProject}.csproj --filter "FullyQualifiedName~{TestClassName}" --verbosity normal
```
