...
**Config:** `__FE_DIR__/playwright.config.ts`
- Test dir: `__FE_DIR__/e2e/specs`
...
**Auth Fixtures:** `__FE_DIR__/e2e/fixtures/auth.fixture.ts`
- `authenticatedPage` — pre-logged-in via `storageState`
- `unauthenticatedPage` — fresh context with auth intercepted
- Auth state saved in `__FE_DIR__/e2e/.auth/e2e-user.json` by the `setup` project
...
2. **Check auth state:**
   ```bash
   ls __FE_DIR__/e2e/.auth/e2e-user.json 2>/dev/null && echo "AUTH EXISTS" || echo "AUTH MISSING"
   ```
   If missing, run setup: `cd __FE_DIR__ && npx playwright test --project=setup`

3. **Run tests:**
   ```bash
   # Full suite
   cd __FE_DIR__ && npx playwright test --project=e2e --reporter=list
   
   # Or specific spec
   cd __FE_DIR__ && npx playwright test e2e/specs/{spec-file}.spec.ts --project=e2e --reporter=list
   
   # Or quality suite
   cd __FE_DIR__ && npx playwright test --project=quality --reporter=list
   ```
...
5. **Handle stale auth:** If tests fail with 401 errors or redirect-to-login patterns, the auth state is stale:
   ```bash
   cd __FE_DIR__ && npx playwright test --project=setup
   ```
...
2. **Find matching existing specs:**
   ```
   Grep: "{page_url_path}" in __FE_DIR__/e2e/specs/
   ```
...
3. **If no matching spec exists**, write a temporary verification spec:
   ...
   b. **Write temp spec** to `__FE_DIR__/e2e/specs/_temp-verify.spec.ts`:
      ```typescript
   ...
   c. **Run the temp spec:**
      ```bash
      cd __FE_DIR__ && npx playwright test e2e/specs/_temp-verify.spec.ts --project=e2e --reporter=list
      ```

   d. **Clean up:**
      ```bash
      rm __FE_DIR__/e2e/specs/_temp-verify.spec.ts
      ```
